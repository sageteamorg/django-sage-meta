...
from django_sage_meta.models import UserData, FacebookPageData, InstagramAccount, Insight, Category, Media, Comment, Story
...
class SyncService:
    """
    Service class to handle synchronization of data from Facebook and Instagram to the local database.
    """

    @staticmethod
    def sync_instagram_accounts():
        """
        Sync Instagram account data including media items associated with each account.
        """
        ...
        SyncService._bulk_sync(insta_objs, InstagramAccount, ['username', 'follows_count', 'followers_count', 'media_count', 'profile_picture_url', 'website', 'biography'])
    ...
    def sync_facebook_pages():
        """
        Sync Facebook page data including categories and associated Instagram business accounts.
        """
        client = FacebookClient(settings.FACEBOOK_ACCESS_TOKEN)
        pages = client.account_handler.get_accounts()

        page_objs = SyncService._process_facebook_pages(pages)

        SyncService._bulk_sync(page_objs, FacebookPageData, ['name', 'categories', 'access_token', 'tasks', 'instagram_business_account_id'])
    ...
    def sync_insights():
        ...
        SyncService._bulk_sync(insight_objs, Insight, ['name', 'period', 'values', 'title', 'description'])
    ...
    def sync_user_data():
        ...
        page = FacebookPageData.objects.get(name=user_info.pages[0].name)
        UserData.objects.update_or_create(
            user_id=user_info.id,
            defaults={'name': user_info.name, 'email': user_info.email, 'pages': page}
        )
    ...
    def sync_media():
        ...
        media_objs, all_comment_objs, media_to_comments = SyncService._process_media(media_list, client)

        SyncService._bulk_sync(media_objs, Media, ['username', 'caption', 'kind', 'media_url', 'timestamp', 'like_count', 'comments_count'])
        SyncService._bulk_sync(all_comment_objs, Comment, ['text', 'username', 'like_count', 'timestamp'])
        ...
        for media_id, comment_id in media_to_comments:
            ...
            media_obj.comments.add(comment_obj)
    ...
    def _process_instagram_accounts(accounts):
        ...
        for account in accounts:
            ...
            insta_obj = InstagramAccount(
                ...
                follows_count=insta_account.follows_count,
                followers_count=insta_account.followers_count,
                media_count=insta_account.media_count,
    ...
    @staticmethod
    def _process_facebook_pages(pages):
        """
        ...
        for page in pages:
            ...
            if page_obj:
                if (
                    page_obj.name != page.name or
                    page_obj.categories != category or
                    page_obj.access_token != page.access_token or
                    page_obj.tasks != page.tasks or
                    page_obj.instagram_business_account_id != page.instagram_business_account
                ):
                    page_obj.name = page.name
                    page_obj.categories = category
    ...
    def _process_insights(insights):
        ...
        for insight in insights:
            ...
            if insight_obj:
                if (
                    insight_obj.name != insight.name or
                    insight_obj.period != insight.period or
                    insight_obj.values != insight.values or
                    insight_obj.title != insight.title or
    ...
    def _process_categories(accounts):
        ...
        for category in categories:
            ...
            if category_obj:
                ...
                category_obj = Category(category_id=category['id'], name=category['name'])
                category_objs.append(category_obj)
    ...
    def _process_media(media_list, client):
        ...
        for media in media_list:
            kind = ContentFileEnum.image if media.media_type == 'IMAGE' else ContentFileEnum.videos
            media_obj = existing_media_dict.get(media.id)
            if media_obj:
                if (
                    media_obj.username != media.username or
                    media_obj.caption != media.caption or
                    media_obj.kind != kind or
                    media_obj.media_url != media.media_url or
                    media_obj.timestamp != media.timestamp or
                    media_obj.like_count != media.like_count or
                    media_obj.comments_count != media.comments_count
                ):
                    ...
                    media_obj.like_count = media.like_count
                    media_obj.comments_count = media.comments_count
            ...
            for comment in comments:
                ...
                if comment_obj:
                    if (
                        comment_obj.text != comment.text or
                        comment_obj.username != comment.username or
                        comment_obj.like_count != comment.like_count or
                        comment_obj.timestamp != comment.timestamp
                    ):
                        comment_obj.text = comment.text
                        comment_obj.username = comment.username
                        comment_obj.like_count = comment.like_count
                        comment_obj.timestamp = comment.timestamp
...
class PublisherService:
    ...
    def publish_media(self, media):
        if media.kind == 'image':
            ...
            self.client.content_publisher.publish_video()
    ...
    def publish_comment(self, comment):
        try:
            self.client.content_publisher.put_comment(comment.media.media_id, comment.text)
